<template>


<section class="page">

  <div class="cells cells-2x">
    <div class="cell cell-table">
      <p class="cell_title">title <i class="cell_icon">placeholder</i></p>
      <div class="cell_table">
        <div class="cell_table_group">
          <div class="cell_table_row">
            <b class="cell_table_value">00</b>
            <p class="cell_table_text">item</p>
          </div>
          <div class="cell_table_row">
            <b class="cell_table_value">00</b>
            <p class="cell_table_text">item</p>
          </div>
        </div>
        <div class="cell_table_modifier">
          <b class="cell_table_value">00</b>
          <p class="cell_table_text">modifier</p>
        </div>
      </div>
    </div>
  </div>

  <div class="cells cells-2x">
    <div class="cell cell-row">
      <p class="cell_title"><i class="cell_icon">placeholder</i> title</p>
      <div class="cell_value cell_value-marked">
        <span class="cell_value_primary">+00</span>
        <i class="cell_mark"></i>
      </div>
    </div>
  </div>

  <div class="cells cells-2x">
    <div class="cell cell-temp cell-column">
      <p class="cell_title">title <i class="cell_icon">placeholder</i></p>
      <div class="cell_value">
        <span class="cell_value_primary">+00</span>
        <span class="cell_value_secondary">(00)</span>
      </div>
    </div>
  </div>

  <div class="cells cells-3x">
    <div class="cell cell-temp cell-column">
      <p class="cell_title">title <i class="cell_icon">placeholder</i></p>
      <input class="cell_input" placeholder="–" type="text">
    </div>
  </div>

  <div class="cells cells-4x">
    <div class="cell cell-temp cell-column">
      <p class="cell_title">title <i class="cell_icon">placeholder</i></p>
      <div class="cell_value">
        <span class="cell_value_primary">+00</span>
        <span class="cell_value_secondary">(00)</span>
      </div>
    </div>
  </div>

  <div class="cells cells-6x">
    <div class="cell cell-temp cell-column">
      <p class="cell_title">title <i class="cell_icon">placeholder</i></p>
      <div class="cell_value cell_value-marked">
        <span class="cell_value_primary">+0</span>
        <i class="cell_mark"></i>
      </div>
    </div>
  </div>



</section>


</template>

<style lang="sass">

  .page 
    gap: 10px
    display: flex
    flex-direction: column

</style>